...
//    private TextView tvEnsure;// 确定按钮显示
    ...
    public DatePicker2(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//        rlTitle.addView(myTitle, lpMyTitle);
    ...
    public void setRightTitle(boolean flag){
        this.isSignIn = flag;
        if(isSignIn){
            mySignIn.setText("今日已签到");
            mySignIn.setTextColor(Color.WHITE);
        }else{
            mySignIn.setText("点此处签到");
            mySignIn.setTextColor(Color.parseColor(DPTheme.RED));
//            signIn.setBackgroundColor(Color.parseColor(DPTheme.RED_BACKGROUND));
        }
    }
